 ...
 * $Date: Fri Dec 03 11:08:01 2010 +0800
...
void GameStateRun::DrawCube(const Cube &cube, const Point &center)
{
	cube.draw(center);
}
...
void GameStateRun::DrawCubeGrid(const Cube::CubeGrid &grid, GLfloat len, GLfloat padding, const Colorf &padding_color)
{
...
#undef TRANS
		...
		// draw center
		glColor4f(grid.color[i].r, grid.color[i].g, grid.color[i].b, grid.color[i].a);
		for (int j = 0; j < 4; j ++)
		{
